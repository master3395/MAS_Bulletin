...
$breakingOrder = MasBreakingLiveTicker::parseIdOrder($pick($params, 'breaking_article_order', ''));
$liveOrder = MasBreakingLiveTicker::parseIdOrder($pick($params, 'live_article_order', ''));

$this->SetPreference('breaking_article_order', MasBreakingLiveTicker::idsToOrderString($breakingOrder));
$this->SetPreference('live_article_order', MasBreakingLiveTicker::idsToOrderString($liveOrder));

$this->SetPreference(
    'breaking_merge_mode',
    MasBreakingLiveTicker::normalizeMergeMode($pick($params, 'breaking_merge_mode', 'manual_only'))
);
$this->SetPreference(
    'live_merge_mode',
    MasBreakingLiveTicker::normalizeMergeMode($pick($params, 'live_merge_mode', 'manual_only'))
);
...
if (function_exists('audit')) {
    audit(0, $this->GetName(), 'MAS_BreakingLive news ticker order updated');
}
